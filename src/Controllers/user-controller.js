...
export const displayProducts = async (req, res) => {
    try {
        // const { name, price, quantity, category } = req.query;
        ...
        const filteredProducts = filterProducts(name, price);
        console.log(date)
        res.json(filteredProducts);

        function filterProducts(name, price) {
            const nameRegex = new RegExp(name, 'i');
            if (name && price) {
                return products.filter(product =>
                    nameRegex.test(product.name) && product.price <= price
                );
            } else if (name) {
                return products.filter(product =>
